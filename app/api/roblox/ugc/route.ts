...
import { getUGCByCreator, getAssetThumbnails, type RobloxAsset } from "@/lib/roblox-api"
...
export async function GET(request: NextRequest) {
  ...
  try {
    const ugcResponse = await getUGCByCreator(Number.parseInt(userId), cursor)

    const assetIds = ugcResponse.data.map((item) => item.id)
    const thumbnailMap = await getAssetThumbnails(assetIds)

    const assets: RobloxAsset[] = ugcResponse.data.map((item) => ({
      id: item.id,
      name: item.name,
      price: item.price,
    ...
    return NextResponse.json({
      data: assets,
      nextPageCursor: ugcResponse.nextPageCursor,
    })
  } catch (error) {
    console.error("Error fetching UGC items:", error)
    logApiRequest(request, 500, startTime, undefined, userId)
    return NextResponse.json({ error: "Failed to fetch UGC items" }, { status: 500 })
  }
}
